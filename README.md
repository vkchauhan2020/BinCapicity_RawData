...
- [ ] Known AR limitation: WebXR hit-test only returns results against surfaces ARCore has detected as a plane (floor, table, or the box's own flat top). Capturing the height corner (point 4, in mid-air above the base) may require resting something flat there, or waiting for ARCore to detect the box's top as a plane, before the crosshair goes green.
